"""
ドライランノード (BitMEX Testnet)

BitMEX Testnet 環境でのドライラン取引ノードを起動します。
バックテストと同一のストラテジーコードを使用します。

Usage:
    python live/dry_run_node.py

環境変数:
    BITMEX_TESTNET_API_KEY: BitMEX Testnet API キー
    BITMEX_TESTNET_API_SECRET: BitMEX Testnet API シークレット
"""
...
def main() -> None:
    testnet_config = get_bitmex_testnet_config()

    config = TradingNodeConfig(
        trader_id=TRADER_ID,
        data_clients={
            BITMEX: testnet_config,
        },
        exec_clients={
            BITMEX: testnet_config,
        },
